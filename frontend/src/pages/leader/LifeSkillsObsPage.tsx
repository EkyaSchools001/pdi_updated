            ...
            <div key={row} className={`grid grid-cols-[1fr_80px_80px_120px] gap-2 items-center px-3 py-3 rounded-lg border ${i % 2 === 0 ? "bg-white" : "bg-slate-50"}`}>
                ...
                {(["Yes", "No", "NA"] as MatrixVal[]).map(val => (
                    <div key={val} className="flex justify-center">
                        <button
                            type="button"
                            onClick={() => setMatrix(field, row, val)}
                            className={cn(
...
const CheckboxGroup = ({
    items, field, label, form, toggleList
}: {
    items: string[];
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {items.map(item => {
                ...
                return (
                    <div
                        key={item}
                        className={cn("flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-all text-sm", checked ? "border-amber-300 bg-amber-50" : "border-slate-200 hover:bg-slate-50")}
                        onClick={() => toggleList(field, item)}
                    >
                                ...
                                <CardContent className="p-6 space-y-5">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {form.observerRole === "Other" && (
                                            <Input placeholder="Specify role..." value={form.observerRoleOther} onChange={e => set("observerRoleOther", e.target.value)} />
                                        )}
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="p-6 space-y-6">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input maxLength={5} placeholder="e.g. A, Emerald" value={form.section} onChange={e => set("section", e.target.value)} className="h-11" />
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        <div className="flex justify-between pt-4 pb-10">
                            ...
                            {step < TOTAL_STEPS ? (
                                ...
                                <Button onClick={handleSubmit} className="gap-2" style={{ background: DARK, color: "white" }}>
                                    <Save className="w-4 h-4" /> Submit Observation
                                </Button>
                            )}
                        </div>
                    </div>
