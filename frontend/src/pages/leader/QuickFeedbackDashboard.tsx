...
const QuickFeedbackDashboard: React.FC = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                setLoading(true);
                const params: any = { type: "Quick Feedback" };
                const res = await api.get("/observations", { params });
                const all = res.data?.data?.observations || res.data?.observations || [];
                setObservations(all.filter((o: any) => o.type === "Quick Feedback" || o.domain === "Quick Feedback"));
            } catch {
    ...
    const newPath = teacherId
        ? `/leader/quick-feedback/new?teacherId=${teacherId}`
        : "/leader/quick-feedback/new";
                ...
                <div className="animate-in fade-in duration-500" style={{ background: "#F5F5EF", minHeight: "100vh", padding: "0 0 40px" }}>
                    ...
                    <Card className="border-none shadow-sm overflow-hidden">
                        <CardHeader className="py-3 px-4 border-b" style={{ background: "#1F2839" }}>
                            <div className="grid grid-cols-6 gap-2 text-xs font-semibold text-white uppercase tracking-wider">
                                <span>Date</span>
                        ...
                        <CardContent className="p-0">
                            {loading ? (
                                ...
                                filtered.map((obs, idx) => (
                                    <div
                                        key={obs.id}
                                        className={`grid grid-cols-6 gap-2 px-4 py-3 items-center hover:bg-amber-50 cursor-pointer border-b transition-colors text-sm ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}
