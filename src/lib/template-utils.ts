    ...
    if (saved) {
        try {
            ...
            const migrated = parsed.map((t: any) => {
                ...
                // Auto-migration: Archive Goal Setting Template (id 3) to force use of custom form
                if (t.id === 3 && t.type === "Goal Setting" && t.status === "Active") {
                    t.status = "Archived";
                    t.type = "Other"; // To match the code change I made earlier to avoid TS errors if type was strict
                    needsUpdate = true;
                }
